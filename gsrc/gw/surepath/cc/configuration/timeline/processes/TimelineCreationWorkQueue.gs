...
class TimelineCreationWorkQueue extends WorkQueueBase<Claim, TimeCreationWorkItem_SP> {
  ...
  override function processWorkItem(workItem : TimeCreationWorkItem_SP) {
    ...
    gw.transaction.Transaction.runWithNewBundle(\bundle -> {
      ...
      //cgi-impl
      if (claim.Timeline_SP != null and claim.Timeline_SP.TimelineStatus == TimelineStatus_SP.TC_RESTORING) {
        TimelineUtil.restoreTimeline(claim.Timeline_SP.RestoreDate, claim)
      } else {
        TimelineUtil.updateTimeline(claim, bundle, true)
      }
