...
uses gw.surepath.cc.configuration.timeline.creation.generators.Cgi_MessagingEventTimelineEntryGenerator
...
class TimelineUtil {
  ...
  static var _generators: List<TimelineEntryGenerator> = new ArrayList<TimelineEntryGenerator>() {
    ...
    new Cgi_MessagingEventTimelineEntryGenerator() //cgi-impl
  }
  ...
  //cgi-impl
  static function getTimelineEntriesForCurrentUpdate(claim : Claim, bundle : Bundle,
                                                     batchRun : boolean = false) : TimelineEntry_SP[] {
    var entries = new ArrayList<TimelineEntry_SP>()
    var reqGenerators = batchRun
        ? _generators.where(\generator -> generator.isOnlyMeantForBatchRun())
        : _generators.where(\generator -> not generator.isOnlyMeantForBatchRun())
    for (s in reqGenerators) {
  ...
  static function updateTimeline(claim : Claim, bundle : Bundle, batchRun : boolean = false) {
    if (claim.Timeline_SP == null) {
      claim.Timeline_SP = new Timeline_SP(bundle)
    }
    ...
    if ((timeline.TimelineStatus == TimelineStatus_SP.TC_ACTIVE) || (!User.util.CurrentUser.isSystemUser())) {
      if (!timeline.isFieldChanged(Timeline_SP#TimelineEntries)) {
        ...
        var entries = getTimelineEntriesForCurrentUpdate(claim, bundle, batchRun)
        if (restore) {
          for (e in entries) {
   ...
   */
  //@IncludeInDocumentation
  static function restoreTimeline(date : Date, claim : Claim) {
    if (claim.Timeline_SP == null) {
